...
class KNNObservationBase(ABC):
    ...
    def __init__(self, idx: int, x: [np.array, float],
        ...
        self.Value = x
        self.MyCluster: [KNNObservation1D] = None
        self._DistanceFunction = distance_function or eucleadian_distance
        self._neighbours: List[KNNObservation1D] = []  # List of K nearest observations
    ...
    def __str__(self):
        return f'KNNObservation1D(ID={self.ID}, Value={self.Value})'
    ...
    def knn_density(self):
        if self._knn_density == 0:
            ...
            k_th_neighbor: KNNObservation1D = self._neighbours[-1]
    ...
    def set_k_nearest_neighbours(self, observations, k, distmatrix: np.array):
        ...
        for other in observations:  # type: KNNObservationBase
            ...
            distances[other] = distmatrix[self.ID, other.ID]
...
class KNNModeSeekerBase(ABC):
    def __init__(self, data: pd.DataFrame, k=2):
        ...
        self.ClusterRoundThreshold = 50  # Used to join neighboring centroids as one cluster
        self._dist = []  # The distance matrix

    @property
    def dist(self) -> np.array:
        if len(self._dist) == 0:
            self._dist = distance_matrix(self.DF.values, self.DF.values)
        return self._dist
    ...
    def _calculate_distances(self):
        ...
        for observation in self.Observations:  # type: KNNObservationBase
            log.info(observation)
            log.info("===============================================")
            observation.set_k_nearest_neighbours(self.Observations, self.K, self.dist)
            self.Densities[str(observation.Value)] = observation.knn_density
    ...
    def _assign_clusters(self):
        ...
        for observation in self.Observations:  # type: KNNObservation1D
            log.info(observation)
            log.info("------------------------------------------")
...
class KNNObservation1D(KNNObservationBase):
    def __init__(self, value, idx: int, distance_function=None):
        super(KNNObservation1D, self).__init__(idx=idx, x=value, distance_function=distance_function)


class KNNModeSeeker1D(KNNModeSeekerBase):
    def __init__(self, data: pd.DataFrame, k=2):
        super(KNNModeSeeker1D, self).__init__(data, k)

    def _save_modes(self):
        self.DF['Mode'] = np.nan
        for o in self.Observations:
            if o.MyCluster is None:
                continue
            self.DF.loc[o.ID, 'Mode'] = o.MyCluster.Value
        log.info('Saving KNN modes to dataframe OK.')

    def _add_cluster(self, mode: KNNObservation1D, observation: KNNObservation1D):
        """Assign an observation to new or existing cluster"""
        ...
        # for c in self.Clusters.keys():  # type: KNNObservation1D
    ...
    def _load_observations(self):
        ...
        for observation in self.DF.itertuples():
            knn_obj = KNNObservation1D(observation.X, idx=int(observation.Index))
...
def _test_1dknn_clustering():
    ...
    knn = KNNModeSeeker1D(data=data, k=60)
...
def _test_2dknn_clustering():
    ...
    data.dropna(inplace=True)
    knn = KNNModeSeeker2D(data=data, k=35)
    knn.run(outfile='data/knnmoderesult2d.csv')
    knn.print_clusters()


def main():
    _test_1dknn_clustering()
    print("\n\n ************** \n\n")
